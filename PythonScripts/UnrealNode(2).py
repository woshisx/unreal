    ...
    def importShots(self, targetDir):
        ...
        for elem in root.iter():
            if elem.tag == 'Camera':
                ...
                level_path = '/Game/%s/shot_work/%s/%s/%s/Lighting/ShotLevel/%s_%s_%s_%s' % (pro, ep, sc,shot,pro, ep, sc,shot)
...
class LayoutImportTools:
    ...
    def addSoundTrack(self, sequence_asset,sound_asset):
        ...
        if sound_track:
            ...
            sound_section = unreal.MovieSceneAudioSection.cast(unreal.MovieSceneTrackExtensions.add_section(sound_track))
    ...
    def importShots(self, targetDir):
        ...
        for elem in root.iter():
            ...
            if elem.tag == 'SkeletalAnimation':
                ...
                Sound_destination_path = '/Game/%s/shot_work/%s/%s/%s/Sound' % (Sound_pro, Sound_ep, Sound_sc,Sound_shot)
        ...
        for elem in root.iter():
            ...
            if elem.tag == 'ShotLevel':
                ...
                dst_ShotLevel = str(unreal.SystemLibrary.convert_to_absolute_path(unreal.Paths.project_content_dir())) + src_ShotLevel.replace("\\","/").split(":")[1].replace("/","",1)
            ...
            if elem.tag == 'Sequencer':
                ...
                dst_Sequencer = str(unreal.SystemLibrary.convert_to_absolute_path(unreal.Paths.project_content_dir())) + src_Sequencer.replace("\\","/").split(":")[1].replace("/","",1)
...
# class UnrealNode():
#     def __init__(self):
#         self.url = 'http://192.168.1.32:5000/ue/task?action=get'

#     def StaticTextureMeshTask(self,xml_dir):
#         print('StaticTextureMeshTask is start')
#         task = StaticTextureMeshTask()
#         # task.importAsset(xml)
#         time.sleep(3)
#     def SkeletonMeshTask(self,xml_dir):
#         time.sleep(10)
#         print('SkeletonMeshTask')
#     def AnimateTask(self,xml_dir):
#         task = ImportShotsTools()
#         task.importShots(xml_dir)
#         time.sleep(10)
#     def LayoutTask(self,xml_dir):
...
#     try:
#         r_json = requests.post(node.url, data_json)
#     except:
...
#             node.StaticTextureMeshTask(r_json.get('XmlFile'))
#         if r_json.get('TaskType') == 'SkeletonMeshTask':
#             node.SkeletonMeshTask(r_json.get('XmlFile'))
#         if r_json.get('TaskType') == 'AnimateTask':
#             node.AnimateTask(r_json.get('XmlFile'))

task = LayoutImportTools()
task.importShots('P:\CSXXM\shot_work\EP001\SC01\P40\Layout\Pulish')